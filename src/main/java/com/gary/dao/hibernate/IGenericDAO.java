...
public abstract interface IGenericDAO<T> {
    ...
    Criteria getCriteria(Class<?> entity);
    /**
     * 获取当前所有实体
     * @return
     */
     ...
     * 获取级联查询
     * @param fetchs 级联属性
     * @param expressions where 条件
     * @param orders 排序
     * @return
     */
    List<T> queryCriteriaFetch(String[] fetchs, SimpleExpression[] expressions, Order[] orders);
     ...
     * @return
     */
    List<T> getSimple(Map<SqlWhere, Object> where);
    
    T getSimpleById(Serializable id);
}
